...
class FieldZones(FROGSubsystem):
    ...
    NO_SHOOT_ZONES = [
        # Blue side trenches
        {
            "x_min": constants.FieldPositions.HubXBlueFacingAlliance,
            "x_max": constants.FieldPositions.HubXBlueFacingCenter,
            "y_min": 0.0,
            "y_max": 1.265,
        },  # Blue Right Trench
        {
        ...
        # Red side trenches
        {
            "x_min": constants.FieldPositions.HubXRedFacingCenter,
            "x_max": constants.FieldPositions.HubXRedFacingAlliance,
            "y_min": 0.0,
            "y_max": 1.265,
        },  # Red Left Trench
        {
    ...
    def get_aim_target(self, pose: Optional[Pose2d] = None) -> Optional[Pose2d]:
        ...
        if alliance == wpilib.DriverStation.Alliance.kRed:
            ...
            if x > constants.FieldPositions.HubXRedFacingCenter:
                return constants.FieldPositions.RedHub
            elif constants.FieldPositions.HubXBlueFacingCenter <= x <= constants.FieldPositions.HubXRedFacingCenter:
                # Middle of the field -> closest Red corner (x=constants.FieldPositions.FieldLength)
                if y < constants.FieldPositions.FieldMidlineY:
                    return constants.FieldPositions.RedRightCorner
                else:
                    return constants.FieldPositions.RedLeftCorner
            else:
                return constants.FieldPositions.RedHub  # Default fallback
        else:
            # Blue Alliance (or fallback): Alliance zone is X < kHubXBlueFacingCenter, opponent zone is X > kHubXRedFacingCenter
            if x < constants.FieldPositions.HubXBlueFacingCenter:
                return constants.FieldPositions.BlueHub
            elif constants.FieldPositions.HubXBlueFacingCenter <= x <= constants.FieldPositions.HubXRedFacingCenter:
                # Middle of the field -> closest Blue corner (x=0)
                if y < constants.FieldPositions.FieldMidlineY:
                    return constants.FieldPositions.BlueRightCorner
                else:
                    return constants.FieldPositions.BlueLeftCorner
            else:
                return constants.FieldPositions.BlueHub  # Default fallback
    ...
    def get_path_for_middle_zone(self, pose: Optional[Pose2d] = None) -> Optional[str]:
        ...
        if not (constants.FieldPositions.HubXBlueFacingCenter <= x <= constants.FieldPositions.HubXRedFacingCenter):
            return None
        ...
        if is_red:
            ...
            is_close = x > constants.FieldPositions.FieldMidlineX
            # Red looks down -X. So +Y (Y > kFieldMidlineY) is to their Right. Left is Y <= kFieldMidlineY.
            is_left = y <= constants.FieldPositions.FieldMidlineY
        else:
    ...
    def get_selection_zone(self, pose: Optional[Pose2d] = None) -> Optional[str]:
        ...
        if is_red:
            if (
                constants.FieldPositions.FieldLength - 3.0 <= x <= constants.FieldPositions.FieldLength
                and constants.FieldPositions.FieldWidth - 2.0 <= y <= constants.FieldPositions.FieldWidth
            ):
    ...
    def get_max_speed_scalar(self) -> float:
        ...
        if self.in_restricted_zone():
            return self.SPEED_SCALAR / constants.Drive.MaxMetersPerSecond
        return 1.0
